...
.comic-card-shell { --comic-stack-depth:14px; position:relative; isolation:isolate; min-width:0; padding-bottom:var(--comic-stack-depth); z-index:1; }
.comic-card-shell::before,.comic-card-shell::after { content:""; position:absolute; z-index:0; height:12px; border:1px solid var(--accent-border,rgba(126,87,194,.22)); border-radius:0 0 var(--radius-sm,8px) var(--radius-sm,8px); background:var(--accent-soft-gradient,var(--accent-soft,#faf7ff)); box-shadow:0 5px 12px color-mix(in srgb,var(--color-theme-glow,var(--theme-glow)) 22%,transparent); pointer-events:none; }
.comic-card-shell::before { right:var(--space-sm,12px); bottom:7px; left:var(--space-sm,12px); }
.comic-card-shell::after { right:var(--space-lg,24px); bottom:1px; left:var(--space-lg,24px); background:var(--accent-soft-gradient,var(--surface-accent,#f0ebff)); }
.comic-card-shell.is-tilting { z-index:3; }
.comic-card { position:relative; z-index:1; isolation:isolate; overflow:hidden; min-width:0; color:inherit; cursor:pointer; border:1px solid var(--color-border-soft,var(--line-soft,rgba(126,87,194,.28))); border-radius:var(--radius-card,22px); background:radial-gradient(circle at top left,color-mix(in srgb,var(--theme-glow-start,var(--theme-glow)) 48%,transparent),transparent 260px),var(--color-surface-card,var(--surface-card,#fff)); box-shadow:var(--shadow-card,0 18px 50px rgba(15,23,42,.14)); transform-style:preserve-3d; will-change:transform; }
.comic-card::after { content:none; }
